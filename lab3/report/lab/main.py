...
def task():
    data1 = load_csv('data/Ch1_600nm_0.23mm.csv')
    data2 = load_csv('data/Ch2_600nm_0.23mm.csv')
    ...
    #plot data together 
    plt.plot(data_n, [data1[i][0] for i in range(len(data1))], label='Ch1_600nm_0.23mm.csv')
    plt.plot(data_n, [data2[i][0] for i in range(len(data2))], label='Ch2_600nm_0.23mm.csv')
    plt.legend()
    ...
    R_interval = [0.000001 * i + 1 for i in range(150000)]
    Jaccars = []
    ...
    optimal_x = opt.fmin(lambda x: -countJaccar(x), 0)
    min1 = opt.root(countJaccar, 1.089)
    max1 = opt.root(countJaccar, 1.10)
    print("min Jaccard", min1.x, "max Jaccard", max1.x)
            
    plt.plot(R_interval, Jaccars, label="Jaccard", zorder=1)
    plt.scatter(optimal_x[0], countJaccar(optimal_x[0]), label="optimal point at R=" + str(optimal_x[0])[0:7],color="r")
    plt.scatter(min1.x, countJaccar(min1.x), label="$min_R$=" + str(min1.x[0])[0:7], color="b", zorder=2)
    plt.scatter(max1.x, countJaccar(max1.x), label="$max_R$=" + str(max1.x[0])[0:7], color="b", zorder=2)
    plt.legend()
    plt.xlabel('$R_{21}$')
    plt.ylabel('Jaccard')
    plt.title('Jaccard vs R')
    plt.savefig("images/Jaccar.png")
    plt.show()
    plt.figure()
